...
search.addEventListener("keyup", function (event) { //do naszej stalej search dodajemy Event i będzimy chcieli po wpisaniu hasla, po zatwierdzeniu enterem czyli event keyup wykonac zapytanie fetch. Czyli chcemy zrobic na poczatku event, w ktorym po zatwierdzeniu enterem pobierzemy wartosc wpisanego hasla i przekazemy ją na backend
    if (event.key === "Enter") { //sprawdzamy w ifie czy faktycznie zostal nacisniety enter, mozemy to zrobic za pomoca event i klucza key, porównujemy to nasteonie z "Enter"
        ...
        fetch("/search", { //nastepnie wykonujemy metode fetch (my już wiemy na podstawie index.php, ze taki fetch bedziemy wykonywali pod url: search)
            ...
            recipeContainer.innerHTML = ""; //w tej metodzie, w tym ostatnim bloku musimy usunac sobie wszystko to co mamy juz wczytane na wstepie do recipesContainera z php czyli wszystkie przepisy musimy sobie wyczyscic, poniewaz my chcemy zaladowac tylko te przepisy, ktore zostaly wyszukane, czyscimy ten contener, ktory pobralismy za pomoca querSelectora opcją innerHTML przypisujac pusty string
            loadRecipes(recipes) //niemniej jednak te wszystkie przepisy, ktore mamy chcemy w jakis sposob wczytac do naszej strony HTML i robimy to w osobnej funkcji loadRecipes do ktorej przekazemy wczytane projekty
        });
    }
});
...
function loadRecipes(recipes) {
    recipes.forEach(recipe => { //po wszystkich przepisach, które otrzymalismy z zapytania na backendzie bedziemt sobie iterowali w petli forEach, bedziemy iterowali po pojedynczym przepisie
        ...
        createRecipe(recipe); // wtym miejsu bedziemy wstawic taki projekt do HTMLa i robimy to w tej funkcji createRecipe
    });
}

function createRecipe(recipe) { //do tej funkcji nie przekazujemy juz calej listy przepisow tylko jeden przepis
    ...
    const template = document.querySelector("#recipe-template"); // za pomoca querySelectora wczytujemy sobie ten template, który przed chwila otrzymalismy odwolujac sie do niego po #klasie

    const clone = template.content.cloneNode(true); //taka templatke mozemy sobie sklonowac za pomoca funkcji clone ale my musimy skopiowac sobie content tego szablonu czyli wszystko to co znajduje się wewnątrz tagu template, mozemy to zrobic za pomoca funkcji cloneNode(), true dlatego zeby zachowac te zagniezdzenia w htmlu (przeszukiwanie w głąb)
